...
class AchievementsMenuState extends MusicBeatState
{
	...
	override function create()
	{
		...
		for (option in options)
		{
			...
			else graphic = Paths.image('achievements/achievement_LOCKED');
	...
	function _changeSelection()
	{
		...
		if (hasProgress)
		{
			...
			progressTxt.text = MathUtil.floorDecimal(val1, options[curSelected].decProgress)
				+ ' / '
				+ MathUtil.floorDecimal(val2, options[curSelected].decProgress);
