...
class CharacterEditorState extends MusicBeatState
{
	...
	{
		if (ClientPrefs.data.cacheOnGPU) FunkinAssets.cache.clearStoredMemory();
	...
	function makeUIMenu()
	{
		...
		setUICam(UI_box, camHUD);
		setUICam(UI_characterbox, camHUD);
	}
	
	function setUICam(obj:FlxUITabMenu, cam:FlxCamera)
	{
		if (obj != null && cam != null) obj.forEach(i -> i.camera = cam, true);
	}
	...
	function addGhostUI()
	{
		...
		var makeGhostButton:FlxButton = new FlxButton(25, 15, "Make Ghost", function() {
			...
			if (!character.isAnimationNull())
			{
				...
				if (spr != null)
				{
					...
					spr.origin.copyFrom(character.origin);
		...
		var ghostAlphaSlider:FlxUISliderEx = new FlxUISliderEx(this, 'ghostAlpha', 10, makeGhostButton.y + 25, 0, 1, 210, 15, 5, FlxColor.WHITE, FlxColor.BLACK);
	...
	function addSettingsUI()
	{
		...
		charDropDown = new FlxUIDropDownMenuEx(10, 30, FlxUIDropDownMenuEx.makeStrIdLabelArray([''], true), function(index:String) {
	...
	function addCharacterUI()
	{
		...
		pauseAnimDuringSustains.checked = character.pauseAnimForSustain;
		pauseAnimDuringSustains.callback = function() {
			character.pauseAnimForSustain = pauseAnimDuringSustains.checked;
		}
	...
	function reloadCharacterImage()
	{
		...
		{
			character.atlas = new OffsetAnimate();
			try
			{
	...
	override function update(elapsed:Float)
	{
		...
		if (!character.isAnimationNull())
		{
			...
			{
				frames = character.atlas.animation.curAnim.curFrame;
				length = character.atlas.animation.curAnim.numFrames;
			}
	...
	function reloadAnimationDropDown()
	{
		...
		animationDropDown.setData(FlxUIDropDownMenuEx.makeStrIdLabelArray(animList, true));
	}
		...
		var json =
			{
				...
				"pauseDuringSustains": character.pauseAnimForSustain
			};
