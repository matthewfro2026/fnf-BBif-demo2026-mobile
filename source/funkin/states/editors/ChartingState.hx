...
class ChartingState extends MusicBeatState
{
	...
	var undos = [];
	var redos = [];
	var eventStuff:Array<Array<String>> = Constants.CHART_EVENTS.copy();
	...
	**/
	...
	override function create()
	{
		...
		if (FileSystem.isDirectory(Paths.getPath('data/${Paths.formatToSongPath(PlayState.SONG.song)}/events/'))) eventStuff.push(['--${PlayState.SONG.song.toUpperCase()}--']);
	...
	function addNoteUI():Void
	{
		...
		var key:Int = 0;
		while (key < Constants.CHART_NOTETYPES.length)
		{
			curNoteTypes.push(Constants.CHART_NOTETYPES[key]);
			key++;
		}
	...
	function addEventsUI():Void
	{
		...
		directories.push(Paths.getPath('data/${Paths.formatToSongPath(PlayState.SONG.song)}/events/'));
