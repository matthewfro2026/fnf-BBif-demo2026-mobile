...
class FunkinVideoSprite extends FlxVideoSprite
{
	...
	public function new(x:Float = 0, y:Float = 0, oneTimeUse:Bool = true)
	{
		...
		if (oneTimeUse) bitmap.onEndReached.add(this.destroy, true, -100);
	}
	...
	/**
	 * Preloads the video to prevent delay.
	 * 
	 * USE BEFORE ANYTHIGN ELSE
	 * 
	 * @param location 
	 * @param options 
	 */
	public function preload(location:Location, ?options:Array<String>):FunkinVideoSprite
	{
		final success:Bool = this.load(location, options);
		if (success)
		{
			this.onStart(() -> {
				this.stop();
