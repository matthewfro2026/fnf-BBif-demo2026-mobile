...
// shadowed from openfl 9.4.1.
// modified to prevent filters increasing the bitmap size by an extra pixel
// changes on line 378
#if !flash
	...
	@:noCompletion private function __updateCacheBitmap(displayObject:DisplayObject, force:Bool):Bool
	{
					...
					|| (renderer.__type == OPENGL && displayObject.scale9Grid != null) #end))
		{
			...
			if (updateTransform || needRender)
			{
				...
				// Remove the + 1 to the width and height
			...
			if (needRender)
			{
				...
				if (displayObject.__cacheBitmapRenderer.__type == OPENGL)
				{
					...
					if (displayObject.__cacheBitmapData.image != null) displayObject.__cacheBitmapData.__textureVersion = displayObject.__cacheBitmapData.image.version + 1;
